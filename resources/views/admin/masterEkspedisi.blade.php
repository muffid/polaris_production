...
<head>
    ...
    <title>Polaris Adv - Master Ekspedisi</title>
</head>
        ...
        <div class="flex flex-col rounded-lg bg-white w-1/3 mx-auto mt-24 shadow-xl relative ">
            <div class="flex flex-row items-center gap-x-4  px-10 bg-slate-100 rounded-t-lg">
                ...
                <div class="flex flex-col p-2 items-start ">
                    <h1 class="text-lg font-bold text-emerald-900">Edit Ekspedisi </h1>
            ...
            <div></div>
            <form action="edit_ekspedisi" method="POST">
                @method('PUT')
                @csrf
                <div class="w-full flex flex-row items-center justify-center gap-x-4 mb-6">
                    <div class="flex flex-col  p-8 text-sm gap-y-4 items-center w-full  bg-white rounded-lg">
                        <div class=" flex flex-row items-center justify-between  gap-x-2 px-4 w-full">
                            <label for="nama_ekspedisi" class="text-left w-1/3 block text-sm font-medium text-gray-700">Nama Ekspedisi</label>
                            <input class="appearance-none border w-full  rounded  py-2 px-3 text-gray-700 leading-tight focus:outline-blue-400 focus:shadow-outline" id="nama_ekspedisi" type="text"  name="nama_ekspedisi" required>
                        </div>
                        <div class=" hidden flex flex-row items-center justify-between  gap-x-2 px-4 w-full">
                            <label for="id_ekspedisi" class="text-left w-1/3 block text-sm font-medium text-gray-700">Nama Bahan</label>
                            <input class="appearance-none border w-full  rounded  py-2 px-3 text-gray-700 leading-tight focus:outline-blue-400 focus:shadow-outline" id="id_ekspedisi" type="text"  name="id_ekspedisi" required>
                        </div>
                            ...
                            <div class="flex flex-col p-6 text-center w-full gap-y-4 mt-14">
                                <div class="flex flex-col  text-center w-full gap-y-4">
                                    <div class="w-full my-4 flex flex-row items-center border-b text-sm">
                                        ...
                                        <a href="master_ekspedisi" class="px-4 py-3   border-b-2  border-blue-700 bg-white rounded-t-xl">
                                    ...
                                    <div class="flex flex-row items-center gap-x-4 mt-2">
                                        ...
                                        <div class="flex flex-col p-2 items-start ">
                                            <h1 class="text-lg font-bold text-emerald-900">Master Ekspedisi</h1>
                                            <p class="text-sm text-slate-400">Masukan Data</p>
                                        </div>
                                    </div>
                                    <form action="save_ekspedisi" method="POST">
                                    @csrf
                                    ...
                                    <div class="bg-white rounded-lg p-8 text-sm w-full lg:w-3/5 2xl:w-2/3">
                                        ...
                                        <table id="data_master_ekspedisi" class="cell-border w-full display nowrap text-left text-sm" style="width:100%">
                                                <thead>
                                                    <tr>
                                                        <th>No</th><th>Nama Ekspedisi</th><th>Aksi</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @for($i=0; $i<sizeof($data_ekspedisi); $i++)
                                                        <tr>
                                                            <td>{{$i+1}}</td>
                                                            <td>{{$data_ekspedisi[$i]->nama_ekspedisi}}</td>
                                                            <td><button type="button" onclick="handleEdit('{{$data_ekspedisi[$i]->nama_ekspedisi}}','{{$data_ekspedisi[$i]->id_ekspedisi}}')" class="px-4 py-1 rounded-full bg-emerald-200 text-emerald-700"><i class="ri-file-edit-line"></i> edit ekspedisi</button></td>
                                                        </tr>
                                                    @endfor
                                                </tbody>
                                            </table>
...
<script>
    var tableDataEcom = new DataTable('#data_master_ekspedisi',{"pageLength":50,});
    function handleEdit(nama_ekspedisi, id_ekspedisi){
    $('#pop_up_edit').removeClass('hidden')
    $('#nama_ekspedisi').val(nama_ekspedisi)
    $('#id_ekspedisi').val(id_ekspedisi)
}
